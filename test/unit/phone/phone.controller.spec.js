...
describe('Phone controller', function() {
  ...
  describe('#find', function() {
    ...
    it('should fail to find a phone if the number is not defined', function(done) {
  ...
  describe('#report', function() {
    ...
    it('should fail to report a phone if the number does not seem to be valid', function(done) {
      phoneController.report({userId: 'dummyToken', phoneNumber: 'abcdef', ad: true, scam: true}).catch(err => {
        expect(err.code).to.be.equal(code.E_DATABASE);
        done();
      });
    });
